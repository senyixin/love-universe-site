...
这是一个 Android WebView 外壳，负责像 App 一样打开服务器上的专属网站。Node 服务、后台管理、照片上传、邮件通知和小票券仍然运行在你的服务器上。
...
进入 GitHub 仓库的 Actions，运行 `Android APK` 工作流，输入服务器地址。打包完成后在 workflow artifacts 里下载 `love-universe-android-debug`。
