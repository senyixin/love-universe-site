...
项目里已经包含 `android-app/`，现在是原生 Android App，不再用 WebView 打开网页。App 会直接请求服务器上的接口，自己绘制暗号入口、天气、互动信号、约会计划、小票券、留言本、留言墙和未来信。后台、照片上传、邮件配置和数据备份仍然由服务器网站处理，安装后桌面名称是「给你的小宇宙」。
